...
int main() {
    string names[MAX_PEOPLE];         // Array to store names
    bool friendships[MAX_PEOPLE][MAX_PEOPLE] = {false}; // 2D array 
    int totalPeople = 0;
    ...
    for (int i = 0; i < totalPeople; i++) {
        for (int j = 0; j < totalPeople; j++) {
            if (friendships[i][j] && !friendships[j][i]) {
                cout << names[i] << " lists " << names[j] << " as a friend,but HE didn't" << endl;
            }
        }
    }
